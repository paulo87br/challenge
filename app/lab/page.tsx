...
async function act(channel:'mail'|'chat',action:string,characterId?:string,extra?:Record<string,unknown>){
 ...
 setRuntimeError('');setQueueNotice('');setBlocked(null);
 ...
 try{
  ...
  for(let attempt=1;attempt<=40;attempt++){
   const r=await fetch('/api/simulation/turn',{method:'POST',headers:{'content-type':'application/json'},
    body:JSON.stringify({world:next,action:acao,sessionId,engine,artifacts,competencies})});
   const data=await r.json();
   if(Array.isArray(data.logs))setEngineLogs(prev=>[...prev,...data.logs].slice(-200));
   if(r.status===202&&data.queued){
    setQueueNotice(data.message||'Aguardando capacidade do modelo.');
    addClientLog('queue','info','Turno na fila',{position:data.position,waitMs:data.waitMs,tentativa:attempt});
    await new Promise(resolve=>setTimeout(resolve,Math.min(60000,Math.max(1500,Number(data.waitMs)||5000))));
    continue;
   }
   setQueueNotice('');
...
async function generateDebrief(){
 ...
 try{
  const r=await fetch('/api/simulation/debrief',{method:'POST',headers:{'content-type':'application/json'},
   body:JSON.stringify({seat:world.seat,evidence,telemetry:world.telemetry,engine,world:{title:world.title,elapsedMinutes:world.minute-initialWorld.minute}})});
  const data=await r.json();
  if(!r.ok)throw new Error(data.detail||data.error||`HTTP ${r.status}`);
...
async function syncSession(patch:{world?:WorldState;debrief?:Debrief|null;status?:string}){
 if(!sessionId)return;
 try{await fetch('/api/session/state',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sessionId,...patch})})}
